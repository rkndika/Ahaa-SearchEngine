...
int main(int argc, char *argv[])
{
  ...
  HEAPSIZE = argc > 2 ? atoi(argv[2]) : (totalDocs > 15 ? 15 : totalDocs);
  ...
  printf("informasi\n");
  /*printf("%s\t%d\t%d\t%ld\t%d\t%f\n","BST",TOTLIST,totalDocs,totalTerm,HEAPSIZE,timingDuration());*/
  printf("%s\t%d\t%ld\t%ld\t%d\t%f\n","BST",TOTLIST,totalDocs,totalTerm,HEAPSIZE,cpu_time_used);

  freeItemTerms(terms,totalTerm);
